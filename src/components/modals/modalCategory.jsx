                ...
                <Modal isOpen={this.state.modal} toggle={this.toggle} className={this.props.className + ' modal-dialog-centered modal-lg'}>
                    ...
                    <ModalBody>
                        ...
                        <div className="row pt-3">
                            ...
                            <div className="col-md-9">
                                <input type="text" className="form-control" name="image_url" onChange={this.handleChange} />
                            </div>
                        </div>
                    </ModalBody>
                    <ModalFooter>
                        <div className="modal-footer" style={{ border: 'none !important' }}>
                            <button type="button" className="btn" style={{ padding: '5px 27px', fontWeight: 'bold', color: 'white', borderRadius: '6px', backgroundColor: '#E28935' }}
                                     onClick={
                                         () => this.props.onAddCategory(this.state.category)
                                        .then(() => { this.setState({ category: { id: Date.now() ,name: '', image_url: '' } }); this.setState({ modal: false }) } ) 
                                     }>Add</button>
                        </div>
                    </ModalFooter>
                </Modal>
